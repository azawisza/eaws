...
package web.service
{
	 ...
	 * */
	public class WSResultParserCNETOld implements WebServiceSubject, WebServiceResultReceiver
	{
		...
		private static var  instance:WSResultParserCNETOld;
		public static  function getInstance():WSResultParserCNETOld{
			logger.info("Obtaining instance of invoker");
			trace("Obtaining instance of invoker"); 
			if(instance==null){
				instance= new WSResultParserCNETOld(new SingletonEnforcer());
			}
				return instance;
		}
		private  var itemAdInit:ArrayCollection= null;
		public var doa:DataObjectAmazon= null;
		
	public function WSResultParserCNETOld(blocker:SingletonEnforcer) {
		...
		//	trace(""+event.result..TechProduct[1].ImageURL.length());
			...
			for(var k:int=0;k<l;k++){
				...
				trace(parsed.toString());
				parsedList.addItem(parsed);
			}	
			//ok parsed
			if(parsedList.length==0){
				logger.warn("NO DATA from CNET ");
				this.data= null;
				return ;
			}
			this.data=parsedList;
			doa = new DataObjectAmazon();
			this.itemAdInit= new ArrayCollection();
